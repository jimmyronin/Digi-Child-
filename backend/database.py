...
def _resolve_db_path():
    """Pick a SQLite path we can actually write to.

    DIGICHILD_DB_PATH is meant to point at a Render persistent disk
    (/data/digichild.db). But disks require a PAID Render instance -- on the
    free tier /data may not exist or be writable, and sqlite3 would raise
    "unable to open database file". Because init_db() runs at import, that
    exception would stop uvicorn from starting and take the whole API down
    over what is only a persistence concern.

    So: verify the configured path is usable and fall back to a local file if
    not. The service always boots; the only cost is that data does not survive
    restarts (attach a disk at the mount point to get persistence back).
    """
    fallback = os.path.join(os.path.dirname(os.path.abspath(__file__)), "digichild.db")
    configured = (os.environ.get("DIGICHILD_DB_PATH") or "").strip()
    if not configured:
        return fallback
    try:
        ...
        probe = sqlite3.connect(configured)  # proves the file can be opened/created
        probe.execute("PRAGMA user_version")
        probe.close()
        return configured
    except (sqlite3.Error, OSError, ValueError) as exc:
        # ValueError covers malformed values that never reach the filesystem at
        # all (e.g. an embedded null byte in the env var). The whole point of
        # this function is that import must never fail, so nothing may escape.
        print(f"[database] DIGICHILD_DB_PATH={configured!r} is not writable ({exc}). "
              f"Falling back to {fallback!r} -- the API will run normally, but data "
              f"will NOT survive restarts. Attach a persistent disk at that mount "
              f"path (paid Render instance) to enable persistence.", flush=True)
        return fallback
